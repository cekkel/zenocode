...
1. **Pick up a task**:
   - Use `backlog task list` to find tasks.
   - Read task details with `backlog task <id>`.
   - Create a new branch: `git checkout -b feature/<task-id>-<short-description>`.
   - Mark task as "In Progress" with `backlog task edit <id> -s "In Progress"`.
2. **Implement the feature**:
   - Develop an implementation plan in the task file.
   - Follow the task's **Implementation Plan** in the task file.
   - Update task acceptance criteria as you complete them.
3. **Testing**:
   - Unit tests in each crate
   - Integration tests in `tests/` directory
4. **Documentation**:
   - Use `backlog task` cli to create tasks for new features or document changes made for a particular feature.
   - Use `backlog decisions` cli to document architectural decisions.
   - Use `backlog doc` cli (see [Backlog.md Guidelines](#backlogmd-guidelines)) for detailed documentation.
5. **Update `AGENTS.md`**:
   - Document new features, patterns, and workflows using `agents-editor.md` to guide the process.
<!-- /workflow_guidlines -->
